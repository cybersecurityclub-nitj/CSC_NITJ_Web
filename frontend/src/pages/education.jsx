...
const EducationPage = () => {
  ...
  const visibleThreats = allThreats.slice(
    currentIndex,
    currentIndex + 3
  );
  ...
  return (
    <div className="bg-[#010714] text-white min-h-screen relative overflow-x-hidden selection:bg-cyan-500/30 font-sans">
      ...
      <section
        id="education-top"
        className="relative z-10 pt-32 pb-4 px-6 max-w-7xl mx-auto"
      >
        <div
        ...
        >
          ...
          <p className="text-gray-400 text-sm md:text-base max-w-xl font-light leading-relaxed italic border-l-2 border-cyan-500/20 pl-6">
            Establishing the baseline for technical resilience through unified
            awareness.
          </p>
        </div>
      </section>
      ...
      <section className="relative z-10 py-8 px-6 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start mb-8">
          ...
          <div className="lg:col-span-2 bg-[#0a1628]/40 backdrop-blur-3xl border border-white/5 p-10 md:p-14 rounded-[3.5rem] shadow-2xl">
            <h2 className="mb-12 flex items-center gap-4">
              <span className="h-[2px] w-12 bg-cyan-500 shadow-[0_0_15px_#22d3ee]" />
              <span className="text-2xl font-black uppercase tracking-[0.4em] text-transparent bg-clip-text bg-gradient-to-r from-white via-white to-cyan-500/50">
                Survival Curriculum
              </span>
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {[
                ...
                <div
                  key={i}
                  className="relative p-8 rounded-[2.5rem] bg-[#0d1b2e]/60 border border-white/5 hover:border-cyan-500/50 hover:bg-[#0d1b2e]/90 transition-all duration-500 cursor-pointer shadow-xl hover:-translate-y-3 hover:scale-[1.03]"
                >
                  <div className="flex items-center justify-between mb-8">
                    <div className="text-4xl">{item.icon}</div>
                    <span className="font-mono text-[10px] text-cyan-500/30 tracking-[0.4em] uppercase">
                      MOD_0{i + 1}
                    </span>
                  </div>

                  <h4 className="font-black text-white uppercase tracking-[0.25em] text-[14px] mb-3">
                    {item.title}
                  </h4>

                  <p className="text-gray-400 text-[12px] leading-relaxed font-light tracking-wide">
                    {item.desc}
                  </p>
                </div>
              ))}
          ...
          <div className="bg-[#0a1628]/60 backdrop-blur-xl border border-white/10 p-10 rounded-[3rem] shadow-2xl flex flex-col min-h-[600px]">
            <h2 className="text-xl font-black mb-12 flex items-center gap-4">
              ...
              <span className="tracking-[0.2em] uppercase text-white/80 text-sm">
                Threat Intelligence
              </span>
            </h2>

            <div
              className={`space-y-10 font-mono flex-grow transition-all duration-500 ${
                isSyncing
                  ? "opacity-0 scale-95"
            ...
            >
              {visibleThreats.map((alert) => (
                <div
                  key={alert.id}
                  className="border-l-2 border-white/5 pl-6 py-2 hover:border-cyan-500/40 transition-colors"
                >
                  <div className="flex justify-between items-end mb-3 text-[10px] uppercase tracking-tighter">
                    ...
                    <span
                      className={`${alert.color} text-[12px] font-black tracking-[0.15em] uppercase`}
                    >
                      {alert.status}
                    </span>
                  </div>
                  ...
                  <p className="text-gray-500 text-[10px] italic uppercase tracking-[0.1em]">
                    Vector: {alert.target}
                  </p>
                </div>
              ))}
            ...
            <button
              onClick={handleSync}
              disabled={isSyncing}
              className="w-full mt-12 py-5 bg-cyan-500/5 border border-cyan-500/20 rounded-2xl font-mono text-[10px] uppercase tracking-[0.4em] text-cyan-500 hover:bg-cyan-500/10 hover:border-cyan-500/50 transition-all duration-300"
            >
              {isSyncing ? "Pulling Data..." : "Synchronize Data"}
            </button>
          </div>
        </div>
